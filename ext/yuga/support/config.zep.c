	...
	if (Z_TYPE_P(key) == IS_ARRAY) {
		...
		if (Z_TYPE_P(key) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(key), _2$$3, _3$$3, _0$$3)
			{
				...
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "set", &_4, 98, &keyItem, &valueItem);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
...
PHP_METHOD(Yuga_Support_Config, delete)
{
	...
	if (Z_TYPE_P(keys) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(keys), _1)
		{
			...
			ZEPHIR_CALL_FUNCTION(&lastSegment$$3, "array_pop", &_7, 99, &segments$$3);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_FUNCTION(&lastSegment$$9, "array_pop", &_7, 99, &segments$$9);
...
PHP_METHOD(Yuga_Support_Config, getIterator)
{
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 100, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
