...
PHP_METHOD(Yuga_Http_Redirect, refresh)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("header", NULL, 50, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Yuga_Http_Redirect, cleanUrl)
{
	...
	ZEPHIR_CALL_FUNCTION(&url, "array_map", NULL, 51, &_1, &secondSection);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_2);
