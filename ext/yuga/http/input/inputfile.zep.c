...
PHP_METHOD(Yuga_Http_Input_InputFile, createFromArray)
{
	...
	ZEPHIR_CALL_METHOD(&_4, &_2, "setsize", NULL, 9, &_5);
	zephir_check_call_status();
...
PHP_METHOD(Yuga_Http_Input_InputFile, getExtension)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("end", NULL, 10, &file);
	ZEPHIR_UNREF(&file);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Yuga_Http_Input_InputFile, move)
{
	...
	ZEPHIR_CALL_FUNCTION(&_0, "is_dir", NULL, 11, destination);
	zephir_check_call_status();
	if (!zephir_is_true(&_0)) {
		ZVAL_LONG(&_1$$3, 0777);
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 12, destination, &_1$$3, &__$true);
		zephir_check_call_status();
	}
	zephir_read_property(&_2, this_ptr, ZEND_STRL("tmpName"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_3);
	ZEPHIR_CONCAT_VSV(&_3, destination, "/", newFilename);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 13, &_2, &_3);
	zephir_check_call_status();
	RETURN_MM();
}
