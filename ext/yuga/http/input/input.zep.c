...
PHP_METHOD(Yuga_Http_Input_Input, parseInputs)
{
	...
	if (zephir_fast_count_int(&_GET) > 0) {
		ZVAL_LONG(&_1$$3, 522);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "filter_var_array", &_3, 10, &_GET, &_1$$3);
		zephir_check_call_status();
	...
	ZVAL_LONG(&_5, 522);
	ZEPHIR_CALL_FUNCTION(&postValues, "filter_var_array", &_3, 10, &_POST, &_5);
	zephir_check_call_status();
	...
	zephir_array_fast_append(&_7, &_8);
	ZEPHIR_CALL_FUNCTION(&_9, "in_array", NULL, 11, &_6, &_7, &__$false);
	zephir_check_call_status();
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_9)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "parse_str", NULL, 12, &_10$$4, &postValues);
	...
	if (Z_TYPE_P(&arrayList) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&arrayList), _2, _3, _0)
		{
			...
			if (Z_TYPE_P(&value) == IS_ARRAY == 0) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_4$$4, "__construct", &_5, 13, &key, &value);
				zephir_check_call_status();
				zephir_array_update_zval(&list, &key, &_4$$4, PH_COPY | PH_SEPARATE);
				continue;
			}
			ZEPHIR_CALL_METHOD(&output$$3, this_ptr, "handlegetpost", &_6, 14, &value);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&value) == IS_ARRAY == 0) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_7$$6, "__construct", &_5, 13, &key, &value);
					zephir_check_call_status();
					zephir_array_update_zval(&list, &key, &_7$$6, PH_COPY | PH_SEPARATE);
					continue;
				}
				ZEPHIR_CALL_METHOD(&output$$5, this_ptr, "handlegetpost", &_6, 14, &value);
				zephir_check_call_status();
...
PHP_METHOD(Yuga_Http_Input_Input, findPost)
{
	...
	if (!(Z_TYPE_P(&stringVar) == IS_NULL)) {
		...
		ZVAL_LONG(&_4$$3, 522);
		ZEPHIR_CALL_FUNCTION(&_5$$3, "filter_var_array", NULL, 10, &_3$$3, &_4$$3);
		zephir_check_call_status();
...
PHP_METHOD(Yuga_Http_Input_Input, all)
{
	...
	if (!ZEPHIR_IS_NULL(&filter)) {
		ZEPHIR_CALL_FUNCTION(&_13, "array_flip", NULL, 15, &filter);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_12, "array_intersect_key", NULL, 16, &output, &_13);
		zephir_check_call_status();
	} else {
