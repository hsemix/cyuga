...
PHP_METHOD(Yuga_Http_Response, cache)
{
	...
	ZEPHIR_CALL_FUNCTION(&_2, "gmdate", NULL, 64, &_1, lastModified);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_3);
	...
	if (!(_7)) {
		...
		if (_8) {
			ZEPHIR_CALL_FUNCTION(&_9, "strtotime", NULL, 65, &httpModified);
			zephir_check_call_status();
...
PHP_METHOD(Yuga_Http_Response, json)
{
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "\is_array", NULL, 66, value);
		zephir_check_call_status();
