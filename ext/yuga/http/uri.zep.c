...
PHP_METHOD(Yuga_Http_Uri, hasParam)
{
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "getparams", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("\in_array", NULL, 77, name, &_0, &__$true);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Yuga_Http_Uri, removeParam)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&key$$3, "\array_search", NULL, 78, name, &params$$3, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Yuga_Http_Uri, parseUrl)
{
	...
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "/[^:\\/@?&=#]+/u");
	ZEPHIR_CALL_FUNCTION(&encodedUrl, "preg_replace_callback", NULL, 79, &_1, &_0, url);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&parts, "parse_url", NULL, 80, &encodedUrl, component);
	zephir_check_call_status();
	...
	ZEPHIR_RETURN_CALL_FUNCTION("array_map", NULL, 71, &_1, &parts);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Yuga_Http_Uri, arrayToParams)
{
	...
	if (!ZEPHIR_IS_NULL(&getParams)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("http_build_query", NULL, 81, &getParams);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_STRING("");
