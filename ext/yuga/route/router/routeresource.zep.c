...
PHP_METHOD(Yuga_Route_Router_RouteResource, hasName)
{
	...
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&_5)) {
		...
		ZVAL_STRING(&_6$$5, ".");
		ZEPHIR_CALL_FUNCTION(&_7$$5, "strrpos", NULL, 88, name, &_6$$5);
		zephir_check_call_status();
		ZVAL_LONG(&_8$$5, 0);
...
PHP_METHOD(Yuga_Route_Router_RouteResource, findUrl)
{
	...
	ZEPHIR_CALL_FUNCTION(&url, "array_search", NULL, 89, name, &_0, &__$false);
	zephir_check_call_status();
...
PHP_METHOD(Yuga_Route_Router_RouteResource, matchRoute)
{
	...
	if (_27) {
		...
		ZVAL_STRING(&_29, "put");
		zephir_array_fast_append(&_28, &_29);
		ZEPHIR_CALL_FUNCTION(&_30, "in_array", NULL, 47, &method, &_28, &__$false);
		zephir_check_call_status();
