...
PHP_METHOD(Yuga_Events_Event, dispatch)
{
	...
	if (Z_TYPE_P(event) == IS_STRING) {
		...
		ZEPHIR_CALL_METHOD(NULL, &eventObject, "__construct", NULL, 40, event, &params);
		zephir_check_call_status();
	}
	zephir_read_property(&_1, this_ptr, ZEND_STRL("attributes"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, &eventObject, "setattributes", NULL, 41, &_1);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_2);
	ZVAL_STRING(&_2, "dispatcher");
	ZEPHIR_CALL_METHOD(NULL, &eventObject, "setattribute", NULL, 42, &_2, this_ptr);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(&_4, &eventObject, "getname", NULL, 43);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_2);
	ZVAL_STRING(&_2, ":");
	ZEPHIR_INIT_VAR(&_5);
	...
	if (!ZEPHIR_IS_FALSE(&_5)) {
		ZEPHIR_CALL_METHOD(&_6$$8, &eventObject, "getname", NULL, 43);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_7$$8, &eventObject, "getname", NULL, 43);
		zephir_check_call_status();
			...
			if (Z_TYPE_P(&list) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&list), _2$$3)
				{
					...
					if (zephir_is_instance_of(callback, SL("Closure"))) {
						ZEPHIR_RETURN_CALL_FUNCTION("call_user_func", &_11, 45, callback, event);
						zephir_check_call_status();
						RETURN_MM();
					}
				} ZEND_HASH_FOREACH_END();
