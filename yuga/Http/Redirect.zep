...
class Redirect
{
    ...
    {
        var url, http;
        var firstSection = explode("://", uri);
        let http = firstSection[0];
        var secondSection = explode("/", firstSection[1]);

        let url = array_map(function (element) {
            return (strpos(element, ":") !== false) ? element : urlencode(element);
        }, secondSection);
        return http . "://". implode("/", url);
    }

    public function httpUrl(string url)
    {
        var cleanUrl;
