...
class Route
{
    ...
    */
     ...
     */
     ...
     {
        let self::defaultNamespace = defaultNamespace;
     }
    ...
    public static function addDefaultNamespace(<IRoute> route)
    {
        if (self::defaultNamespace !== null) {
            ...
            if (callback === null || callback[0] !== "\\") {
                ...
                var namespaceValue = self::defaultNamespace;

                var currentNamespace = route->getNamespace();

                if (currentNamespace !== null) {
                    let namespaceValue .= "\\" . currentNamespace;
                }

                route->setDefaultNamespace(namespaceValue);
    ...
    */
    ...
    {
        return self::match(["patch"], url, callback, settings);
    }
    ...
    */
    ...
    {
        return self::match(["options"], url, callback, settings);
    }
    ...
    */
    ...
    {
        var route;
        let route = new RouteUrl(url, callback);
        route->setRequestMethods(requestMethods);
        let route = self::addDefaultNamespace(route);
    ...
    */
    ...
    {
        var route;
        let route = new RouteController(url, controller);
        let route = self::addDefaultNamespace(route);
    ...
    {
        var routes, group, callbackException;

        let routes = self::router()->getRoutes();

        let callbackException = new CallbackException(callback);
    ...
    */
    public static function resources(url, controller, array settings = null)
    {
        var route;
        let route = new RouteResource(url, controller);
        let route = self::addDefaultNamespace(route);
    ...
    */
    ...
    // {
    //     self::router()->setCsrfVerifier(baseCsrfVerifier);
    // }
    ...
    */
    public static function response()
    {
        if (self::response === null) {
            let self::response = new Response(self::request(), new Redirect(self::request()));
        }
